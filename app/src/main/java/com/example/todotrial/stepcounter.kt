...
class stepcounter : AppCompatActivity(), SensorEventListener {
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        ...
        if (running) {
            ...
            if(currentSteps==10 || currentSteps==0 ||currentSteps ==100)
                Toast.makeText(this,"Come On Run More",Toast.LENGTH_LONG).show()

            if(currentSteps==1000)
                Toast.makeText(this,"Just 1000 Steps..Run more",Toast.LENGTH_LONG).show()

            if(currentSteps==7000)
            {
                Toast.makeText(this,"Bss ab Check krdo",Toast.LENGTH_LONG).show()
            }
